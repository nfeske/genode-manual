...
\newcommand\rpcobjectnode[4] {
	\node[#2] (#1) {
		\begin{tikzpicture}
			\node [outer sep=0pt, inner color=white, fill opacity=1.0, dropshadow, capability]
			       (#1cap) {#3};
			\node [draw, left color=blue!20, right color=white, minimum size=0pt,
			       thick, inner sep=1ex, rounded corners=0.5ex, dropshadow]
			       (#1obj) [right=2ex] {#4};
			\path [draw, dropshadow, very thick] (#1cap.east) -- (#1obj);
		\end{tikzpicture}
	};
}
...
\newcommand\kernelboundary[1]{
	...
	\draw [dashed, very thick, color=black!20!red!100] (kernelboundary.north west)
